...
function bootstrap() {
	...
	add_filter( 'dbdelta_queries', __NAMESPACE__ . '\\filter_dbdelta_queries' );

	// Update the table name on the $wpdb global when switching sites in a multisite environment.
	add_action( 'switch_blog', __NAMESPACE__ . '\\register_collaboration_table' );
...
/**
 * Filter the queries generated by dbDelta to create the collaboration table.
 *
 * This creates the custom table upon the creation of a new site on a multisite instance.
 *
 * Runs on the `dbdelta_queries` filter.
 *
 * @param array $queries The queries generated by dbDelta.
 * @return array The modified list of queries.
 */
function filter_dbdelta_queries( $queries ) {
	if ( ! is_multisite() ) {
		return $queries;
	}
